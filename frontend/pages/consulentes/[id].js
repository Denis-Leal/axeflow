...
export default function PerfilConsulente() {
  ...
  useEffect(() => {
    ...
    api.get(`/membros/consulentes/${id}/perfil`)
      .then(r => setPerfil(r.data))
      .catch(() => setErro('Consulente não encontrado'))
      ...
      <div style={{ display:'flex' }}>
        ...
        <div className="main-content">
          ...
          <div className="topbar">
            ...
            <Link href="/consulentes" style={{ color:'var(--cor-texto-suave)', textDecoration:'none', fontSize:'0.9rem' }}>← Voltar</Link>
          </div>
          ...
          <div className="page-content">
            ...
            <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1.5rem' }}>
              <span style={{ background:scoreCor.bg, border:`1px solid ${scoreCor.border}`, color:scoreCor.text,
                borderRadius:'20px', padding:'4px 14px', fontSize:'0.8rem', fontWeight:600 }}>
                {sc?.emoji} {sc?.label}{sc?.score != null ? ` — ${sc.score}%` : ''}
              </span>
              <span style={{ background:`${retorno.cor}18`, border:`1px solid ${retorno.cor}40`, color:retorno.cor,
                borderRadius:'20px', padding:'4px 14px', fontSize:'0.8rem', fontWeight:600 }}>
              ...
              {sc?.alerta && (
                <span style={{ background:'rgba(249,115,22,0.1)', border:'1px solid rgba(249,115,22,0.3)',
            ...
            {/* Métricas */}
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(148px, 1fr))', gap:'0.75rem', marginBottom:'1.5rem' }}>
              ...
              <div className="stat-card">
                ...
                <div className="stat-value" style={{ color: perfil.faltas >= 3 ? '#ef4444' : 'var(--cor-texto)' }}>{perfil.faltas}</div>
              ...
              <div className="stat-card">
                ...
                <div style={{ fontSize:'0.72rem', color:'var(--cor-texto-suave)' }}>
                  {perfil.primeira_data && perfil.ultima_visita && perfil.primeira_data !== perfil.ultima_visita
                    ? (() => {
                        const meses = Math.round((new Date(perfil.ultima_visita) - new Date(perfil.primeira_data)) / (1000*60*60*24*30));
            ...
            <div className="card-custom">
              <div className="card-header">
                <span style={{ fontFamily:'Cinzel', fontSize:'0.9rem', color:'var(--cor-acento)' }}>✦ Histórico de Giras</span>
              ...
              <div style={{ padding:'1.25rem' }}>
                ...
                {perfil.historico.map((h, idx) => {
                  const s = STATUS_RETORNO[h.status] || {};
                  const scGira = {
                    ...
                    <div key={`${h.gira_id}-${idx}`} style={{ display:'flex', gap:'1rem', paddingBottom: isLast ? 0 : '1.1rem' }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'0.25rem' }}>
                          <div>
                            <Link href={`/giras/${h.gira_id}`}
                              style={{ color:'var(--cor-texto)', textDecoration:'none', fontWeight:600, fontSize:'0.9rem' }}>
                              {h.gira_titulo}
                            </Link>
                            {h.gira_tipo && (
                              <span style={{ marginLeft:'6px', fontSize:'0.7rem', color:'var(--cor-texto-suave)',
                          ...
                          <span style={{ fontSize:'0.75rem', color:'var(--cor-texto-suave)', whiteSpace:'nowrap' }}>
                            {new Date(h.data + 'T00:00:00').toLocaleDateString('pt-BR', { day:'2-digit', month:'short', year:'numeric' })}
                          </span>
                        </div>
                        <div style={{ display:'flex', gap:'0.5rem', marginTop:'3px', alignItems:'center' }}>
                          ...
                          <span style={{ fontSize:'0.7rem', color:'var(--cor-texto-suave)' }}>· {h.posicao}º na lista</span>
                        </div>
                      </div>
                    </div>
                  );
