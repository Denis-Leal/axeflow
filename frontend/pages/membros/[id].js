/**
 * pages/membros/[id].js — AxeFlow
 *
 * Perfil completo de um membro do terreiro.
 * Exibe score de presença, métricas e histórico de giras —
 * espelhando o padrão da tela consulentes/[id].js, adaptado
 * para o contexto de membros (sem fila de espera, sem score público).
 *
 * Acesso: qualquer membro autenticado do mesmo terreiro pode ver
 * o perfil (os dados são internos ao terreiro).
 */
...
// Labels e cores para o status de engajamento do membro
const STATUS_RETORNO = {
  ativo:            { label: 'Ativo',            cor: '#10b981', emoji: '🟢', desc: 'Compareceu nos últimos 60 dias' },
  morno:            { label: 'Morno',            cor: '#f59e0b', emoji: '🟡', desc: 'Entre 60 e 180 dias sem comparecer' },
  inativo:          { label: 'Inativo',          cor: '#ef4444', emoji: '🔴', desc: 'Mais de 180 dias sem comparecer' },
  nunca_compareceu: { label: 'Nunca compareceu', cor: '#94a3b8', emoji: '⚫', desc: 'Confirmou mas nunca compareceu' },
};

// Ícones por status de inscrição
const ICONE_STATUS = {
...
export default function PerfilMembro() {
  ...
  if (erro || !perfil) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <Link href="/membros" style={{ color: 'var(--cor-acento)' }}>← Voltar</Link>
      </div>
    </div>
  );
  ...
  const taxaPresenca = finalizadas > 0
    ? Math.round((perfil.comparecimentos / finalizadas) * 100)
    : null;
        ...
        <div className="main-content">
          ...
          <div className="topbar">
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div style={{
                ...
                fontFamily: 'Cinzel', color: 'var(--cor-acento)', fontSize: '1.2rem',
              ...
              <div>
                ...
                <small style={{ color: 'var(--cor-texto-suave)' }}>
                  {/* Role colorido + email */}
                  <span style={{ color: ROLE_COLORS[perfil.role] || 'var(--cor-texto-suave)' }}>
                    {ROLES[perfil.role] || perfil.role}
                  </span>
                  {perfil.email && (
                    <span style={{ marginLeft: '0.5rem' }}>· {perfil.email}</span>
                  )}
                </small>
              </div>
            </div>
          ...
          <div className="page-content">
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
              ...
              {/* Alerta de faltante crônico */}
              {sc?.alerta && (
                <span style={{
                  background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.3)',
                  color: '#f97316', borderRadius: '20px', padding: '4px 14px',
            ...
            <div style={{
              ...
              <div className="stat-card">
                <div style={{ fontSize: '0.7rem', color: 'var(--cor-texto-suave)', marginBottom: '2px' }}>
                  Comparecimentos
                </div>
                <div className="stat-value" style={{ color: '#10b981' }}>
                  {perfil.comparecimentos}
                </div>
                <div style={{ fontSize: '0.72rem', color: 'var(--cor-texto-suave)' }}>
                  confirmados pelo admin
                </div>
              </div>
              ...
              {/* Taxa de presença com barra visual */}
              <div className="stat-card">
                <div style={{ fontSize: '0.7rem', color: 'var(--cor-texto-suave)', marginBottom: '4px' }}>
                  Taxa de presença
                </div>
                {taxaPresenca !== null ? (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <div className="vagas-bar" style={{ flex: 1 }}>
                        <div className="vagas-fill" style={{
                          width: `${taxaPresenca}%`,
                          background: taxaPresenca >= 80 ? '#10b981'
                            : taxaPresenca >= 50 ? '#f59e0b'
                            : '#ef4444',
                        }} />
                      </div>
                      <span style={{
                        fontSize: '1rem', fontWeight: 700,
                        color: taxaPresenca >= 80 ? '#10b981'
                          : taxaPresenca >= 50 ? '#f59e0b'
                          : '#ef4444',
                      }}>
                        {taxaPresenca}%
                  ...
                  {perfil.primeira_data && perfil.ultima_visita && perfil.primeira_data !== perfil.ultima_visita
                    ? (() => {
                        const meses = Math.round(
                          (new Date(perfil.ultima_visita) - new Date(perfil.primeira_data))
                          / (1000 * 60 * 60 * 24 * 30)
                        );
                        return meses > 0
                          ? `${meses} ${meses === 1 ? 'mês' : 'meses'} de histórico`
                          : 'mesmo mês';
                      })()
                    : `membro desde ${new Date(perfil.cadastrado_em).toLocaleDateString('pt-BR')}`}
                </div>
              </div>
            ...
            <div className="card-custom">
              ...
              <div style={{ padding: '1.25rem' }}>
                ...
                {perfil.historico.map((h, idx) => {
                  // Cor do item da timeline baseada no status da inscrição
                  const scGira = {
                  ...
                  return (
                    <div
                      key={`${h.gira_id}-${idx}`}
                      style={{ display: 'flex', gap: '1rem', paddingBottom: isLast ? 0 : '1.1rem' }}
                    >
                      {/* Ícone do status + linha vertical da timeline */}
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                        ...
                        {!isLast && (
                          <div style={{
                            width: '2px', flex: 1, background: 'var(--cor-borda)',
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ marginTop: '3px' }}>
                          ...
                          {h.posicao && (
                            <span style={{ fontSize: '0.7rem', color: 'var(--cor-texto-suave)', marginLeft: '0.5rem' }}>
